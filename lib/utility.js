...
async function checkPermission(header, permission) {
    ...
    userPermission = userPermission[0].role.permissions;
    const permissions = Object.keys(userPermission);
    const permissionName = permissions[permission];
    if (!userPermission[permissionName]) {
        throw {
            status: 403,
            message: "Forbidden",
            why: "You don't have permission to do this action"
        };
    }
}
